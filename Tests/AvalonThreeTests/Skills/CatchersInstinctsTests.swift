...
struct CatchersInstinctsTests {
    ...
    @Test func noFreeRunActionOfferedOnCatchingHandoff() async throws {
        ...
        (latestEvents, latestAddressedPrompt) = try game.process(
            InputMessageWrapper(
                coachID: .away,
                message: .sidestepActionSelectSquare(square: sq(3, 7))
            )
        )
    ...
    @Test func freeRunActionDeclined() async throws {
        ...
        #expect(latestAddressedPrompt?.coachID == .away)
        #expect(latestAddressedPrompt?.prompt.case == .eligibleForCatchersInstinctsSkillRunAction)
    ...
    @Test func doesNotErrorOnCatchingForTouchdown() async throws {
